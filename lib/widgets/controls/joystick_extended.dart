...
import 'package:climbing_alien/widgets/custom_slider.dart';
...
class _JoystickWithButtonAndSliderState extends State<JoystickWithButtonAndSlider> {
  ...
  void processCallback(Offset offset) {
    ...
    double angle = _math.atan2(
        shiftedOffset.dy, shiftedOffset.dx); // Calculates the angle in radians between x-axis and the point.
  ...
  Widget build(BuildContext context) {
    return _buildJoystickWithSliderBasedOnSliderSide(widget.sliderSide);
  }

  _buildJoystickWithSliderBasedOnSliderSide(SliderSide sliderSide) {
    switch (sliderSide) {
      case SliderSide.LEFT:
        return Row(children: [
          RotatedBox(quarterTurns: 3, child: _buildSlider()),
          _buildJoystick(),
        ]);
      case SliderSide.RIGHT:
        return Row(children: [
          _buildJoystick(),
          RotatedBox(quarterTurns: 1, child: _buildSlider()),
        ]);
      case SliderSide.TOP:
